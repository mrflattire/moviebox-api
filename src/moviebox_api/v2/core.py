...
class SearchWithFilter(Search):
    ...
    anime_filter_params = ("country", "sort", "year")
    ...
    @property
    def _channel_id(self) -> tuple[bool, int]:
        return SUBJECT_TYPE_CHANNEL_ID_MAP[self._subject_type]
...
class SingleItemDetails(BaseItemDetails):
    ...
    async def get_content(self, path_or_item: str | SearchResultsItem) -> dict:
        ...
        if isinstance(path_or_item, SearchResultsItem):
            if not SubjectType.is_movie_like(path_or_item.subjectType):
                raise ValueError(
                    "item's subjectType has to be one of"
                    f"{SubjectType.get_movies_like_subject_types()!r} "
...
class TVSeriesDetails(BaseItemDetails):
    """Fetch specific item details - tv_series and alike"""
    ...
    async def get_content(self, path_or_item: str | SearchResultsItem) -> dict:
        ...
        if isinstance(path_or_item, SearchResultsItem):
            if not SubjectType.is_series_like(path_or_item.subjectType):
                raise ValueError(
                    f"item's subjectType has to be one of"
                    f"{SubjectType.get_series_like_subject_types()!r}"
                    f"not {path_or_item.subjectType!r}"
                )

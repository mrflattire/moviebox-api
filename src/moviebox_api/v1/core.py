...
class Search(BaseSearch):
    ...
    async def get_content(self) -> dict:
        ...
        if self._subject_type in {SubjectType.TV_SERIES, SubjectType.MOVIES}:
            target_items = []
...
class MovieDetails(BaseItemDetails):
    ...
    def __init__(self, url_or_item: str | SearchResultsItem, session: Session):
        ...
        if isinstance(url_or_item, SearchResultsItem):
            if not SubjectType.is_movie_like(url_or_item.subjectType):
                raise ValueError(
...
class TVSeriesDetails(BaseItemDetails):
    ...
    def __init__(self, url_or_item: str | SearchResultsItem, session: Session):
        ...
        if isinstance(url_or_item, SearchResultsItem):
            if not SubjectType.is_series_like(url_or_item.subjectType):
                raise ValueError(
